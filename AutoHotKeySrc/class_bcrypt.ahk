; MsgBox % bcrypt.hash("The quick brown fox jumps over the lazy dog", "SHA256")
...
class bcrypt
{
	...
	hashToBin(String, AlgId, ByRef bytes, encoding := "utf-8")
	{
		AlgID         := this.CheckAlgorithm(AlgID)
		ALG_HANDLE    := this.BCryptOpenAlgorithmProvider(AlgID)
		OBJECT_LENGTH := this.BCryptGetProperty(ALG_HANDLE, this.BCRYPT_OBJECT_LENGTH, 4)
		...
		; FileAppend, %HASH_LENGTH% `n, *
		loop % HASH_LENGTH {
			x := NumGet(HASH_DATA , A_Index - 1, "uchar")
			; FileAppend, %A_Index% `n, *
			NumPut(x, bytes, A_Index - 1, "uchar")
		}
